...
resource "aws_lambda_function" "s3transitionexecutor_resource" {
  ...
  source_code_hash = "${base64sha256(file(var.s3transitionexecpkgname))}"
  environment {
    variables  {
      ES_HOSTS = "https://${aws_elasticsearch_domain.s3accessstore.endpoint}"
    }
  }
  depends_on = ["aws_iam_role_policy_attachment.s3transtionexecutor_logs", "aws_cloudwatch_log_group.s3transitionexecutor_log_group"]
...
resource "aws_lambda_function" "s3transitioner_resource" {
  ...
  source_code_hash = "${base64sha256(file(var.s3transitionpkgname))}"
  environment {
    variables  {
      ES_HOSTS = "https://${aws_elasticsearch_domain.s3accessstore.endpoint}"
  ...
  depends_on = ["aws_iam_role_policy_attachment.s3transtioner_logs", "aws_cloudwatch_log_group.s3transitioner_log_group"]
  timeout = "${var.lambda_timeout}"
}
...
resource "aws_lambda_function" "s3accessmonitor_resource" {
  ...
  source_code_hash = "${base64sha256(file(var.s3accessmonitorpkgname))}"
  environment {
    variables  {
      ES_HOSTS = "https://${aws_elasticsearch_domain.s3accessstore.endpoint}"
    }
  }
  depends_on = ["aws_iam_role_policy_attachment.s3accessmonitor_logs", "aws_cloudwatch_log_group.s3accessmonitor_log_group"]
  timeout = "${var.lambda_timeout}"
}
